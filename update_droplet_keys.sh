...
# If token file exists then use that
if [[ "$TOKEN" == "" && -f "$DIR/.token" ]]; then
      TOKEN=`cat "$DIR/.token"`
fi
...
for name in ${NAMES[@]}; do
   echo "# Public Key for $name (from DigitalOcean)" >> "$TEMP_FILE"
   echo "${KEYS[${COUNTER}]}" | tr -d "\"" >> "$TEMP_FILE"
   if [[ `echo "${KEYS[${COUNTER}]}" | tr -d "\""` == "$MY_SSH_KEY" ]]; then
      THISKEY="YES"
   fi
   COUNTER=$[$COUNTER + 1]
done
...
if [[ "$THISKEY" == "NO" ]]; then
   ...
   echo "# Public Key for $MY_HOSTNAME (ONLY STORED LOCALLY)" >> "$TEMP_FILE"
